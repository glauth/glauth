...
func NewPluginHandler(log *logging.Logger, cfg *config.Config, yubikeyAuth *yubigo.YubiAuth) handler.Handler {
	...
	db, err := sql.Open("mysql", cfg.Backend.Database)
	if err != nil {
...
func (h mysqlHandler) CreateSchema() {
	statement, _ := h.database.cnx.Prepare(`
...
`)
	statement.Exec()
	statement, _ = h.database.cnx.Prepare("CREATE UNIQUE INDEX idx_user_name on users(name)")
	statement.Exec()
	statement, _ = h.database.cnx.Prepare("CREATE TABLE IF NOT EXISTS groups (id INTEGER AUTO_INCREMENT PRIMARY KEY, name VARCHAR(64) NOT NULL, unixid INTEGER NOT NULL)")
	statement.Exec()
	statement, _ = h.database.cnx.Prepare("CREATE UNIQUE INDEX idx_group_name on groups(name)")
	statement.Exec()
	statement, _ = h.database.cnx.Prepare("CREATE TABLE IF NOT EXISTS includegroups (id INTEGER AUTO_INCREMENT PRIMARY KEY, parentgroupid INTEGER NOT NULL, includegroupid INTEGER NOT NULL)")
	statement.Exec()
}

func (h mysqlHandler) GetPrepareSymbol() string {
	return "?"
}

func (h mysqlHandler) Bind(bindDN, bindSimplePw string, conn net.Conn) (resultCode ldap.LDAPResultCode, err error) {
	...
	err = h.database.cnx.QueryRow(fmt.Sprintf(`
			...
			FROM users u WHERE u.name=%s`, h.GetPrepareSymbol()), userName).Scan(
	...
	group := config.Group{}
	err = h.database.cnx.QueryRow(fmt.Sprintf(`
			SELECT g.unixid FROM groups g WHERE name=%s`, h.GetPrepareSymbol()), groupName).Scan(
		&group.UnixID)
	if err != nil {
...
func (h mysqlHandler) Search(bindDN string, searchReq ldap.SearchRequest, conn net.Conn) (result ldap.ServerSearchResult, err error) {
	bindDN = strings.ToLower(bindDN)
	baseDN := strings.ToLower("," + h.cfg.Backend.BaseDN)
...
func (h mysqlHandler) Close(boundDn string, conn net.Conn) error {
	stats.Frontend.Add("closes", 1)
	return nil
}

func (h mysqlHandler) intToBool(value int) bool {
	if value == 0 {
		return false
	}
	return true
...
func (h mysqlHandler) commaListToTable(commaList string) []int {
	if len(commaList) == 0 {
		return make([]int, 0)
	}
...
func (h mysqlHandler) memoizeGroups() ([]config.Group, error) {
...
func (h mysqlHandler) getGroupMembers(gid int) []string {
...
func (h mysqlHandler) getGroupMemberIDs(gid int) []string {
	members := make(map[string]bool)
	rows, err := h.database.cnx.Query(`
			SELECT u.name,u.unixid,u.primarygroup,u.passsha256,u.otpsecret,u.yubikey,u.othergroups
			FROM users u`)
...
func (h mysqlHandler) getGroupDNs(gids []int) []string {
...
// Invoked for every user being returned from our database
func (h mysqlHandler) getGroupName(gid int) string {
	for _, g := range h.MemGroups {
		if g.UnixID == gid {
			return g.Name
		}
...
// Toolbox
func (h mysqlHandler) getGroup(g config.Group) *ldap.Entry {
...
func (h mysqlHandler) getAccount(u config.User) *ldap.Entry {
	attrs := []*ldap.EntryAttribute{}
