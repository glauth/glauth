...
func newSqliteHandler(cfg *config, yubikeyAuth *yubigo.YubiAuth) Backend {
	...
	if err != nil {
		log.Fatalf("Unable to open sqlite database named '%s'", cfg.Backend.Database)
	}
...
func (h sqliteHandler) Bind(bindDN, bindSimplePw string, conn net.Conn) (resultCode ldap.LDAPResultCode, err error) {
...
func (h sqliteHandler) Search(bindDN string, searchReq ldap.SearchRequest, conn net.Conn) (result ldap.ServerSearchResult, err error) {
	bindDN = strings.ToLower(bindDN)
	baseDN := strings.ToLower("," + h.cfg.Backend.BaseDN)
...
func (h sqliteHandler) Close(boundDn string, conn net.Conn) error {
	stats_frontend.Add("closes", 1)
	return nil
}

func (h sqliteHandler) intToBool(value int) bool {
	if value == 0 {
		return false
	}
	return true
...
func (h sqliteHandler) commaListToTable(commaList string) []int {
	if len(commaList) == 0 {
		return make([]int, 0)
	}
...
func (h sqliteHandler) memoizeGroups() ([]configGroup, error) {
...
func (h sqliteHandler) getGroupMembers(gid int) []string {
...
func (h sqliteHandler) getGroupMemberIDs(gid int) []string {
	members := make(map[string]bool)
	rows, err := h.database.cnx.Query(`
			SELECT u.name,u.unixid,u.primarygroup,u.passsha256,u.otpsecret,u.yubikey,u.othergroups
			FROM users u`)
...
func (h sqliteHandler) getGroupDNs(gids []int) []string {
...
// Invoked for every user being returned from our database
func (h sqliteHandler) getGroupName(gid int) string {
	for _, g := range h.MemGroups {
		if g.UnixID == gid {
			return g.Name
		}
